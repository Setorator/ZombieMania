...
public class GameComponent extends JComponent implements BoardListener
{
    ...
    //contains information about the color of each se.liu.ida.davsc542kimla207.tddd78.project.zombiemania.BlockType
    public GameComponent(final Board board) {
	this.board = board;
	this.heart = ResourceLoader.loadImageResource("Resources/Pictures/heart.png").getImage();
	imageMap = new EnumMap<>(BlockType.class);
	this.background = ResourceLoader.loadImageResource("Resources/Pictures/background.gif").getImage();
	...
	imageMap.put(BlockType.GROUND, ResourceLoader.loadImageResource("Resources/Pictures/grassblock.png").getImage());
	imageMap.put(BlockType.BOX, ResourceLoader.loadImageResource("Resources/Pictures/woodblock.png").getImage());
	imageMap.put(BlockType.SPIKE, ResourceLoader.loadImageResource("Resources/Pictures/spikes.png").getImage());
	imageMap.put(BlockType.STARTSIGN, ResourceLoader.loadImageResource("Resources/Pictures/start.png").getImage());
	imageMap.put(BlockType.INSTRUCTIONS, ResourceLoader.loadImageResource("Resources/Pictures/startsign.png").getImage());
	imageMap.put(BlockType.FINISH, ResourceLoader.loadImageResource("Resources/Pictures/gateway.gif").getImage());
	imageMap.put(BlockType.DIRT, ResourceLoader.loadImageResource("Resources/Pictures/dirtblock.png").getImage());
    }
	...
	for (int y = 0; y < board.getMap().length; y++) {
		...
		if (board.getBlockType(x, y).equals(BlockType.GROUND) || board.getBlockType(x, y).equals(BlockType.BOX) ||
		    ...
		    // when we finds a creep in the map we creates a se.liu.ida.davsc542kimla207.tddd78.project.zombiemania.Creep and adds it to our array of creeps.
